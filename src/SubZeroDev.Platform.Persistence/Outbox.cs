...
internal sealed class OutboxStore(IAmbientTransactionAccessor ambient, IProviderCapability capability) : IOutboxStore
{
    public async Task<Result<TransactionError>> InsertAsync(OutboxMessage message, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // sequence is provider-allocated claim order, not an identity: the scalar subquery
            // reproduces SQLite's own rowid-reuse behaviour after a drain and prune (MAX of an empty
            // table is null, so the next row starts back at 1), and is race-free here because a
            // write transaction on SQLite already serialises against every other writer.
            command.CommandText = """
