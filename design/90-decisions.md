...
- **`TraceContextCodec.StartRoot` does not start a root when telemetry is recording** — an S8 defect, surfaced by `OperationScopeTests.Disposing_a_scope_restores_the_one_it_replaced` failing intermittently in CI. The origin branch passes `parentContext: default` to force "a genuinely new root rather than silently becoming that scope's child", and its comment says exactly that. A default `ActivityContext` is not "no parent" to .NET — an invalid parent context makes `StartActivity` fall back to `Activity.Current`, so a nested origin scope inherits the outer scope's trace id and therefore its correlation. Confirmed on net10.0 with a standalone repro, both branches: with a listener the two trace ids are identical, and suppressing `Activity.Current` around the call makes them differ.
  **Why the failure looks intermittent.** With no listener recording, `StartActivity` returns `null`, both scopes fall through to `Mint(ActivityTraceId.CreateRandom())`, and the ids differ — the test passes. Activity listeners are process-global, so whether another test host has OTel running concurrently decides the outcome. This was previously written off as a flake that "passes on re-run"; the re-run passing was listener timing, not correctness. **The test is right and the product is wrong** — origin scopes silently nest under one another exactly when telemetry is switched on, which is the configuration that matters.
  **Why not fixed here.** Product code, and this reconcile is docs-only — *one slice at a time*.
- **A second, unrelated intermittent CI failure, cause not established** — `TelemetryExportTests.A_blocked_OTLP_collector_never_makes_a_request_wait_and_export_resumes_after_release` timed out with `System.TimeoutException` at `TelemetryExportTests.cs:80`, the twenty-second wait for the exporter's first batch to reach the gated collector. Observed once, on `dc74c42`; the `StartRoot` failure above passed on that same run, and this one passed on the run before it. Two runs of the branch failed the suite at 254/255, but not on the same test.
  **This is recorded at the strength of the evidence, which is one run and a stack trace.** The plausible reading is a test-side assumption about the OTLP exporter's scheduled delay holding under CI load, but nothing here establishes that, and it must not be filed as though it were the characterised defect above — that one has a repro and this one does not. **Do not close it on a green re-run**: the entry above is exactly what that reasoning got wrong once already.
  **Why not fixed here.** Uncharacterised, and open-ended product debugging does not belong in a docs reconcile.
