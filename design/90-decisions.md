...
- **GitHub Pages is not enabled**, and the deploy workflow now installed expects it. `.github/workflows/docs-deploy.yml` triggers on push to `main` and needs Pages set to build from GitHub Actions; until that is switched on in repository settings the deploy job will fail. Repository settings are the owner's, not an agent's. The docs checks should also be made required, or a red run will not block a merge.
