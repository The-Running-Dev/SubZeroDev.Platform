...
- **A slice that ships without touching `design/30-slices.md` leaves the public site wrong, and
  the marker gate passes.** `site/src/roadmap/roadmapData.ts` parses each slice's `**Status:**`
  line, so a merged slice still marked `in progress` or `queued` is advertised as unstarted.
