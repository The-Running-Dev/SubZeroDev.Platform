...
>
> **Earned here:** *Documentation site*, every *Verification* entry, and the two marker and
> producer lessons under *Drift* and *Verification*. Those cost failed fixes, wrong diagnoses
> and a shipped release to find.
...
- **A slice that ships without touching `design/30-slices.md` leaves the record wrong, and
  the marker gate passes.** A merged slice still marked `in progress` or `queued` is recorded as
  unstarted. (During D3 the cost surfaced publicly: `site/src/roadmap/roadmapData.ts` parsed this
  file and the site advertised the stale marker. It now parses the archived
  `design/d3/30-slices.md`, and issue #80 tracks pointing the roadmap back at the active effort —
  the lesson is about the marker, not the site.)
  `build/Test-SliceStatusMarkers.ps1` cannot catch it: its invariants only forbid *more than
  one* `in progress`, zero `in progress` beside a `queued`, and `shipped` ordered after
  `queued` — none of which compares a marker against a merged commit. **Cost: paid twice. S7
  was found by `/track` and fixed on 2026-08-06; S8 and S9 then repeated it, so `v0.1.0` shipped
...
- **A test that proves a safety net works needs inputs wide enough to trip it.** The
  ...
  reads as coverage.
- **A public well-known name with no producer reads as implemented, and every gate agrees.**
  `PlatformTelemetry.MeterName` shipped, Observability subscribed to it with `.AddMeter`, it
  carried a doc comment, it appeared in the generated API reference, and S8's metric-allowlist
  criterion passed — while **nothing in `src/` ever constructed a `Meter` or an instrument.**
  Each gate checked something adjacent: the reference gate checks that public types are
  *documented*, never that anything emits, and the allowlist test found a built-in ASP.NET Core
  instrument to assert against. **Cost: three design commitments — a poison metric, a
  backlog-age metric, and per-Platform-instrument label allowlists — were unimplemented through
  S8's nine criteria, S9's release gate and a shipped `v0.1.0`, and surfaced only in a reading
  pass.** It would have changed a decision: a criterion written against a Platform instrument
  rather than "each standard metric" forces one to exist. When a design promises a signal, ask
  what *produces* it, not whether the name is there.
