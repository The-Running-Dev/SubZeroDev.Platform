...
| Tarball | Built from |
|---|---|
| `subzerodev-service-contract-0.6.0.tgz` | `SubZeroDev.ServiceContract`, `npm pack` |
| `the-running-dev-game-engine-0.10.0.tgz` | `SubZeroDev.GameEngine`, `npm pack` (byte-identical to the copy vendored in `SubZeroDev.ServiceContract`) |
...
`SubZeroDev.ServiceContract` generates its `0.6.0` contract against the engine's `0.10.0` release
(`listSaves`/`branchSession`/`deleteSave` join the session store, thirteen operations up from
ten), so this repository's own engine pin moves to the same `0.10.0` in the same commit —
otherwise every startup fails `EngineVersionMismatch` before the listener ever binds.
