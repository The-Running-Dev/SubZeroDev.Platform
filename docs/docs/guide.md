...
asserts `read committed` on connect and refuses to start against anything else, rather than
discovering the problem the first time two writers race.
...
| A stored row exists but its columns can't be read back into the shape the store expects (corruption, or a blob from an incompatible engine version) | `internal_failure` | `500` | Not caller-fixable. The row's `engine_version` column lets an operator tell version skew from real corruption after the fact. |
...
configuration, so tests can shrink them to seconds.
...
rows only once they are past a much larger **retention horizon** (also 30 days past expiry, by
default). The gap between "expired" and "actually deleted" exists on purpose: it's what lets the
wire answer "this session existed and is gone" (`session_expired`, 404) instead of "no idea"
(`unknown_session`, also 404 but a different code) for as long as the retained row survives. Past
the retention horizon, the answer honestly degrades to the second one — the row is gone, and nothing
distinguishes an expired id from one that never existed.
...
**Starting up.** On boot, the workload binds its listener and reports **live** immediately, then
runs migrations to head under the migration tool's own advisory lock (safe for two instances
starting together), connects, and only then reports **ready**. Between those two points it is up
and answering — every request just fails with `storage_failure`, which is the same answer a
connected-but-degraded store gives, so there's no separate "not started yet" behaviour for a caller
to learn.
...
it exists to catch. The edge's own readiness probe was changed to match — it checks the workload's
*readiness* endpoint rather than its liveness endpoint, because with a durable store a workload can
be alive and unable to serve anything.
...
- **No performance tuning.** No connection-pool sizing, no latency target, no benchmark presented as
  ...
  second.
- **One store implementation the engine's ports.** The workload's own database client and
...
**One seam is deliberately left for later work to use, not for this workload to use itself:** every
one of the three storage ports (`SessionPersistence`, `ProfileStore`, and the lifecycle probe used
internally for expiry classification) is composed through a single decorator seam
(`composeStorageSeam`), so a future authorization layer can wrap all three at once instead of
discovering piecemeal that one of them — the lifecycle probe in particular, since it can answer
whether *any* id is live, expired, or absent — was left uncovered. Nothing decorates them today; the
identity decorator is the only one that runs.
